...
export default function AllOrMyPacksToggle() {
  ...
  return (
    <Box
      component="form"
      sx={{
        '& > :not(style)': { m: 1, display:"flex", justifyContent:'space-between' },
      }}
      noValidate
      autoComplete="off"
    >
      ...
      <div>
      <Button variant="outlined" className={s.button} onClick={handleChangeMyClick}>MY</Button>
      <Button variant="contained" className={s.button} onClick={handleChangeAllClick}>ALL</Button>
      </div>
    </Box>
  );
}
