...
import { CardGradeType, CardType } from 'features/cards/service/cards.api.types';
...
const Stars = (grade:any) => {
  let arr = StartGrade(grade)
   return <div>{arr.map((grade) => (
    grade === 1 ? <StarIcon/> : <StarBorderIcon />
   ))}</div>
}
        ...
        <TableBody>
          {props.cards && props.cards.map((card) => (
            ...
            >
              ...
              <TableCell align="right"><Stars grade={card.grade} /></TableCell>


              <TableCell align="right"  sx={{display:'flex', flexDirection:'row',justifyContent:'center'}}>
                <IconButton aria-label="learn" onClick={() => {handleNavigateToLearn(props.packID)}}>
                    <SchoolIcon />
                </IconButton>
                {userID === card.more_id 
                  ? <> 
                    <EditCardModal card={card} key={card._id}/>
                    <DeleteCardModal card={card} page={props.page} pageCount={props.pageCount} />
                  </>
                  : <></>
                }
              </TableCell>
