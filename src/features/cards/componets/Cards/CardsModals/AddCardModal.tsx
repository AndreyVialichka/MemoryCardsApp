...
import { useAddCardMutation } from 'features/cards/service/cards.api';
...
export default function AddCardModal() {
  ...
  return (
    <div>
      <Button variant="contained" aria-label="create" onClick={handleEditModalOpen}>
                    ADD CARD
       </Button>
      <Modal
        open={openEditModal}
      ...
      >
        ...
        <Box sx={{display:'flex', flexDirection:'column',gap:'40px', alignContent:"center", justifyContent:"center"}}>
          ...
          <Divider />
          <TextField id="standard-basic"  variant="standard" label ="Question" value={cardQuestion} onChange={handleCardQuestionChange}/>
          <TextField id="standard-basic"  variant="standard" label ="Answer" value={cardAnswer} onChange={handleCardAnswerChange}/>
          <Box sx={{display:'flex', justifyContent:'space-between',width:"100%"}}>
            <Button variant="contained" disabled={cardQuestion.length === 0 || cardAnswer.length === 0} onClick={() => {
                addCardHandler()
